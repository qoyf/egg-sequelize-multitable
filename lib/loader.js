...
module.exports = app => {
  const defaultConfig = {
    ...
    logging(...args) {
      ...
      app.logger.info('[egg-sequelize-multitable]%s %s', used, args[0]);
    },
    host: 'localhost',
    port: 3306,
    username: 'root',
  ...
  const config = app.config.sequelizeMultitable;
  // support customize sequelize
  app.Sequelize = config.Sequelize || require('sequelize');
  // app.SequelizeMultitableType = require("./model");
  const databases = [];
  ...
  function loadDatabase(config = {}) {
    if (typeof config.ignore === 'string' || Array.isArray(config.ignore)) {
      app.deprecate(`[egg-sequelize-multitable] if you want to exclude ${config.ignore} when load models, please set to config.sequelize.exclude instead of config.sequelizeMultitable.ignore`);
    ...
    if (model[delegate[len - 1]]) {
      throw new Error(`[egg-sequelize-multitable] app[${config.delegate}] is already defined`);
    }
    ...
    const DELEGATE = Symbol(`context#sequelize-multitable_${config.delegate}`);
    ...
    app.loader.loadToApp(modelDir, target, {
      ...
      initializer(factory) {
        if (typeof factory === 'function') {
          return factory(app, sequelize, config);
        }
      },
    });
